...
public final class MoveGenerator {
  ...
  private void generateCaptures() {
    ...
    for (int i = 0; i < tempChessmanList.size; i++) {
      int position = tempChessmanList.position[i];
      addDefaultCaptureMovesTo(board.board[position], position, moveDeltaKnight, IntPosition.NOPOSITION);
    }
    tempChessmanList = board.bishopList[activeColor];
    for (int i = 0; i < tempChessmanList.size; i++) {
      int position = tempChessmanList.position[i];
      addDefaultCaptureMovesTo(board.board[position], position, moveDeltaBishop, IntPosition.NOPOSITION);
    }
    tempChessmanList = board.rookList[activeColor];
    for (int i = 0; i < tempChessmanList.size; i++) {
      int position = tempChessmanList.position[i];
      addDefaultCaptureMovesTo(board.board[position], position, moveDeltaRook, IntPosition.NOPOSITION);
    }
    tempChessmanList = board.queenList[activeColor];
    for (int i = 0; i < tempChessmanList.size; i++) {
      int position = tempChessmanList.position[i];
      addDefaultCaptureMovesTo(board.board[position], position, moveDeltaQueen, IntPosition.NOPOSITION);
    }
    assert board.kingList[activeColor].size == 1;
    int position = board.kingList[activeColor].position[0];
    addDefaultCaptureMovesTo(board.board[position], position, moveDeltaKing, IntPosition.NOPOSITION);
  }
  ...
  private void generateEvasion(Attack attack) {
    ...
    for (int i = 0; i < tempChessmanList.size; i++) {
      ...
      if (!board.isPinned(position, activeColor)) {
        addDefaultCaptureMovesTo(board.board[position], position, moveDeltaKnight, attackerPosition);
      }
    }
    tempChessmanList = board.bishopList[activeColor];
    for (int i = 0; i < tempChessmanList.size; i++) {
      int position = tempChessmanList.position[i];
      if (!board.isPinned(position, activeColor)) {
        addDefaultCaptureMovesTo(board.board[position], position, moveDeltaBishop, attackerPosition);
      }
    }
    tempChessmanList = board.rookList[activeColor];
    for (int i = 0; i < tempChessmanList.size; i++) {
      int position = tempChessmanList.position[i];
      if (!board.isPinned(position, activeColor)) {
        addDefaultCaptureMovesTo(board.board[position], position, moveDeltaRook, attackerPosition);
      }
    }
    tempChessmanList = board.queenList[activeColor];
    for (int i = 0; i < tempChessmanList.size; i++) {
      int position = tempChessmanList.position[i];
      if (!board.isPinned(position, activeColor)) {
        addDefaultCaptureMovesTo(board.board[position], position, moveDeltaQueen, attackerPosition);
      }
    }
